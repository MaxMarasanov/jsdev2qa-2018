...
function validateReport() {
    ...
    else if (document.getElementById("manufacturer").value === "iphone" || document.getElementById("manufacturer").value === "samsung" || document.getElementById("manufacturer").value === "nokia") {

        var manufacturerSelectedValue = document.getElementById("manufacturer").value;
        var modelSelectedValue = document.getElementById("model").value;

        //при дефолтной модели сразу же вызываем создание репорт с нужными параметрами
        if (modelSelectedValue === "default") {
            generateReport(manufacturerSelectedValue, modelSelectedValue);
        }

        //иначе делаем попытку найти модель у заданного производителя
        else {
            var modelArray = manufacturerSelectedValue + "Models";
            var finalArray = window[modelArray];
            ...
            for (var i = 0; i < finalArray.length; i++) {
                ...
                //если модель нашли - создаем репорт с ней, перестаем искать
    ...
    //если выбранный производитель вообще не существует
    else {
        var errorMessage = document.getElementsByClassName("result")[0];
...
function generateReport(manufacturerSelectedValue, modelSelectedValue) {

    //если репорт создан успешно, то обнуляем текст ошибки
    var errorMessage = document.getElementsByClassName("result")[0];
    ...
    //иначе выводим инфу по всем моделям указанного производителя
        ...
        for (var i = 0; i < finalArray.length; i++) {
            ...
            var row2 = table.insertRow(rowNumber);

            var cell2 = row2.insertCell(0);
            var cell3 = row2.insertCell(1);

            var cellText2 = document.createTextNode(finalArray[i]);
            cell2.appendChild(cellText2);

            var cellText3 = document.createElement('div');

            //если нечетное кол-во символов - статус доступен
            if (finalArray[i].length % 2 !== 0) {
                cellText3.className = 'available';
                cellText3.innerHTML = "Available";
            }
